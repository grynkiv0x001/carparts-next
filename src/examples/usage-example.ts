...
const _multipleParts = generateCarParts(10);
console.log('\nGenerated 10 parts');

// Example 4: Generate unique parts (no duplicate part numbers)
const _uniqueParts = generateUniqueCarParts(50);
...
const brakeParts = sampleProducts.filter((part) =>
  part.categoryId.startsWith('cat-2'),
);
...
const camry2019Parts = sampleProducts.filter((part) =>
  part.compatibleModels.some(
    (model) => model.modelId === 'model-001' && model.years.includes(2019),
  ),
);
...
const inStockParts = sampleProducts.filter((part) => part.availability.inStock);
